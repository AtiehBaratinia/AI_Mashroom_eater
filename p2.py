...
def huristic():
    return min_manhatan_remaining_mashrooms()


"""this method calculate the minimum manhatan distance to remaining mashrooms"""


def min_manhatan_remaining_mashrooms():
    least = float("inf")
    for i in blue_mashrooms:
        dif_x = abs(i.x - current_location.x)
        dif_y = abs(i.y - current_location.y)
        if least > dif_x + dif_y:
            least = dif_y + dif_x
    return least
